...
let rec lookup ide table =
  ...
  | Table (parent, hash_tbl) -> (
      try Hashtbl.find hash_tbl ide with Not_found -> lookup ide parent)
...
let add_entry ide value table =
  ...
  | Table (parent_block, hash_tbl) -> (
      try
        let _ = Hashtbl.find hash_tbl ide in
        raise (DuplicateEntry ide)
      with Not_found ->
        let _ = Hashtbl.add hash_tbl ide value in
        Table (parent_block, hash_tbl))
...
let rec scan_list l table =
  ...
  | [] -> table
  | (ide, value) :: xs ->
      let nt = add_entry ide value table in
      scan_list xs nt

let of_alist list table = scan_list list table
